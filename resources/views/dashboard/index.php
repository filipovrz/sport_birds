<h1>Табло <small style="font-weight:normal;color:var(--muted)">v<?= htmlspecialchars($appVersion ?? '2.0.0') ?></small></h1>
<?php if (!$isPremium): ?><p class="alert" style="background:#fff3cd">Безплатен план — <a href="/dashboard/subscription">надградете</a> за GPS, карта и обяви.</p><?php endif; ?>
<div class="grid grid-3">
    <div class="card stat-card"><div class="num"><?= (int)$stats['birds'] ?></div><div>Птици</div></div>
    <div class="card stat-card"><div class="num"><?= (int)$stats['lofts'] ?></div><div>Птичарници</div></div>
